module multicycle_cpu(
	...
	output [15:0] debug_instr_d,
	
	output [7:0] debug_sel_3_d,
	output [7:0] debug_x,
	output [7:0] debug_y,
	output [7:0] debug_alu_res,
	
	output debug_ena_alu,
	output debug_ena_x,
	output debug_ena_y,
	output debug_ena_d,
	output debug_ena_c
	...
	wire [3:0] fl;
	wire [3:0] conz;
	wire [7:0] x, y, sel_3_d, alu_res;
	wire [15:0] instr_d;
	wire ena_alu, ena_x, ena_y, ena_d, ena_c;
	...
	assign debug_instr_d = instr_d;
	
	assign debug_sel_3_d = sel_3_d;
	assign debug_x = x;
	assign debug_y = y;
	assign debug_alu_res = alu_res;
	
	assign debug_ena_alu = ena_alu;
	assign debug_ena_x = ena_x;
	assign debug_ena_y = ena_y;
	assign debug_ena_d = ena_d;
	assign debug_ena_c = ena_c;	
		
	cmem co(clk,reset,con[1],pc_out,sel_1[5:0],sw,instr_d);
	
	register #(16) cm(clk,ena_c,instr_d,instr);
	...
	control cnt(clk,reset,ohe[1],ohe[2],ohe[3],ohe[4],ohe[5],ohe[6],ohe[7],ohe[8],ohe[9],ohe[10],ohe[11],ohe[12],ohe[13],ohe[14],ohe[15],ohe[16],ohe[17],ohe[18],ohe[19],ohe[20],ohe[21],ohe[22],ohe[23],conz[2],conz[1],conz[0],instr[11:10],instr[9:8],con,ena_alu,ena_x,ena_y,ena_d,ena_c);
	
	alu a(opd_1,opd_2,{con[12],con[13]},alu_res,fl[0],fl[3],fl[2],fl[1]);
	
	register al(clk,ena_alu,alu_res,alures);
	
	mux2 alur(alures,instr[7:0],con[15],sel_1);
	
	mux2 alus(temp_1,instr[7:0],con[11],opd_2);
	
	regfile rf(clk,reset,con[10],{con[4],con[5]},{con[6],con[7]},{con[8],con[9]},sel_2,x,y,debug_regs);
	
	register xy_1(clk,ena_x,x,opd_1);
	
	register xy_2(clk,ena_y,y,temp_1);
	
	flag f(clk,con[14],reset,fl,conz);
	
	dmem da(clk,reset,con[17],sel_1[3:0],temp_2,sel_3_d,out);
	
	register dam(clk,ena_d,sel_3_d,sel_3);
